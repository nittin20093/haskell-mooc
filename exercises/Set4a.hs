...
middle x y z = if x < y && y < z || z < y && y < x then y else if y < x && x < z || z < x && x < y then x else z
...
incrementKey k [] = []
incrementKey k ((key,value):kv) = if k == key then (key, value + 1) : incrementKey k kv else (key,value) : incrementKey k kv
...
winner scores player1 player2 = if (Map.findWithDefault 0 player1 scores) < (Map.findWithDefault 0 player2 scores) then player2 else player1
...
freqs xs = foldr (\ x -> Map.insertWith (+) x 1) Map.empty xs
...
transfer from to amount bank = if Map.member from bank && Map.member to bank && amount >= 0 && Map.lookup from bank >= Just amount then
    Map.insert from (maybe (Map.findWithDefault 0 from bank) (subtract amount) (Map.lookup from bank))
    (Map.insert to (maybe (Map.findWithDefault 0 to bank) (+ amount) (Map.lookup to bank)) bank)
    else bank
