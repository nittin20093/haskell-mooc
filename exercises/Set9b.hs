...
prettyPrint n queens = concat [ [ if (r,c) `elem` queens then 'Q' else '.' | c <- [1..n] ] ++ "\n" | r <- [1..n] ]
...
prettyPrint2 n s = concat [ [ if (r,c) `elem` s then 'Q' else if danger (r,c) s then '#' else '.' | c <- [1..n] ] ++ "\n" | r <- [1..n] ]
...
fixFirst n ((r,c):s) = if c > n then Nothing else if danger (r,c) s then fixFirst n ((r,c+1):s) else Just ((r,c):s)
...
continue [] = []
continue (x:s) = nextRow x : (x:s)

backtrack :: Stack -> Stack
backtrack [] = []
backtrack [_] = []
backtrack (x:y:s) = nextCol y : s
