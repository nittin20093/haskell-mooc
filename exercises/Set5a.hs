...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix color1 color2) = mixColors (rgb color1) (rgb color2)
rgb (Invert color) = invertColors (rgb color)

mixColors :: [Double] -> [Double] -> [Double]
mixColors [] [] = []
mixColors (c1:cs1) (c2:cs2) = ((c1 + c2) / 2) : mixColors cs1 cs2

invertColors :: [Double] -> [Double]
invertColors [] = []
invertColors (c:cs) = (1 - c) : invertColors cs
