...
readWords n = do
    line <- getLine
    rest_lines <- readWords (n-1)
    return (sort (line : rest_lines))
...
isumsHelper sum 0 = return sum
isumsHelper sum n = do
    num <- readLn
    print (sum + num)
    isumsHelper (sum + num) (n - 1)


isums :: Int -> IO Int
isums n =  isumsHelper 0 n
