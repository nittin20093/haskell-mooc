...
mapMaybe2 f Nothing (Just x) = Nothing
...
firstHalf s = if length (s) `mod` 2 == 0 then take (length (s) `div` 2) s else take ((length (s) + 1) `div` 2) s
...
capitalizeFirst s = map toUpper (take 1 s) ++ tail s
...
joinToLength n li = [a ++ b | a <- li , b <- li , length(a ++ b) == n]
