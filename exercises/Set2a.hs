...
takeFinal :: Int -> [a] -> [a]
takeFinal n xs = if (length xs - n) < 0 then xs else drop (length xs - n) xs
...
palindromify s = if s == reverse s then s else palindromify (drop 1 (take (length s - 1) s))
...
safeIndex xs i = if i < 0 || i >= length xs then Nothing else Just (head (drop i xs))
