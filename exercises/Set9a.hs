...
countValid [] = 0
countValid (x:xs) = if x !! 2 == x !! 4 || x !! 3 == x !! 5 then 1 + countValid xs else 0+countValid xs
...
sumSuccess (x:xs) =
    case x of
      Left "no data" -> Right 0
      Left _ -> sumSuccess xs
      Right v1 -> case sumSuccess xs of
            Left _ -> Right v1
...
open code (Closed lockedCode) = if code == lockedCode then Open lockedCode else Closed lockedCode
open _ lock = lock
...
compose f g = [(x, c) | (x, y) <- f, Just c <- [lookup y g]]
