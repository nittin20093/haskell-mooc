...
split [] = Nothing
split (x:xs) = if x == ' '  then Just ("", xs) else case split xs of
  Just (forename, surname) -> Just (x:forename, surname)
  Nothing -> Nothing
...
checkNumber (forename, surname) = if (any isDigit forename || any isDigit surname) then Nothing else Just (forename, surname)
...
checkCapitals (x:for,y:sur) = if isUpper x && isUpper y then Just (x:for,y:sur) else Nothing
...
safeIndex (_:xs) i = if i < 0 then Nothing else safeIndex xs (i - 1)

selectSum :: Num a => [a] -> [Int] -> Maybe a
selectSum xs is = sum <$> traverse (\i -> safeIndex xs i) is
...
countAndLog p xs = do
    let filtered_msgs = filter p xs
    mapM_ (msg . show) filtered_msgs
    return (length filtered_msgs)
...
balance accountName = BankOp findBalance 
  where findBalance (Bank acc) = (Map.findWithDefault 0 accountName acc, Bank acc)
...
paren c = do
  state <- get
  if c == '(' && state /= (-1) then modify (+1)
  else if c == ')' && state /= (-1) then modify (subtract 1)
  else return ()
