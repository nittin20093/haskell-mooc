...
ifM opBool opThen opElse = do
    boolResult <- opBool
    if boolResult==True then opThen else opElse
...
visit maze place = do
    state <- get
    if place `elem` state then return ()
    else do
        modify (place:)
...
path maze place1 place2 = let (x , state) = runState (visit maze place1) [] in place2 `elem` state
...
findSum2 ks ns = do
  ...
  j <- ks
  let n = i + j
  if (n `elem` ns) then return (i, j, n) else []
...
allSums :: [Int] -> [Int]
allSums xs = [sum x | x <- subsequences xs]
...
f2 acc x = do
  state <- get
  if x `elem` state then return acc
  else do
    modify (x:)
    return (acc + x)
