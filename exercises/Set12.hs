...
count xs = foldr (\x acc -> if x == xs then acc + 1 else acc) 0
...
inBoth xs ys = foldr (\x acc -> if elem x (toList ys) then x : acc else acc) [] xs
...
instance Foldable TwoList where
  foldr _ y TwoEmpty = y
  foldr f y (TwoNode x z xs) = f x (f z (foldr f y xs))
